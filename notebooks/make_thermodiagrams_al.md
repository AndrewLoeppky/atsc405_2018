...
pres = np.arange(10000, 110000, 10000)  # Pa

# initialize figure
fig, ax = plt.subplots(figsize=(9,9)) 
plt.gca().invert_yaxis()
ax.set_yscale("symlog")
ax.set_yticks(pres, pres // 100) # convert to kPa here
ax.set_ylim(104000, 10000)

# plot pressures
[ax.axhline(p, color="k", alpha=0.3) for p in pres];
```

## Task 2 - Plot Isotherms

Put temperatures ranging from -40 to +50 $^o$C along the x axis. 
...
Write a function that implements this equation on input arrays for $T$ and $p$. Re-initialize your plot and re-do parts 2 and 3 (isotherms and the sounding), using a skew parameter $\alpha= 20$

```{code-cell} ipython3
def T_to_skewT(T, p, alpha):
    """
    skews T by angle alpha
    """
...
Add saturated adiabats to your plot! The calculation here is trickier, feel free to use functions from `thermlib.py`.
